...
# rack also tells teh app/framework to:
...
# enable concurrency
require "thread"
require "eventmachine" # server coding here is simpler because it takes care for us the socket logic (eg. Tcp SERVER INSATNIATION, RAED AND WRITE SOCKET)
# when you're inside an event loop and when you block the event loop with blocking operations like making it lengthy  SQL calls to the database and you block it for 3 seconds.. then the server will be blocked for 3 seconds.. but the upside is that you will get great performance for very little memory
# event machine uses forks so it will mess up the code
...
class Tube
  ...
  def initialize(port, app)
    ...
    @server = TCPServer.new(port) # this is the SERVER SOCKET # 5this is the master process
    @app = app # now we have access to the rack application instance inside our Connection 
  end

  def prefork(workers)
    workers.times do
      fork do # fork creates a coipy of the current process, attach that copy to the current processs, and share the sockets with that child process
        puts "Forked #{Process.pid}" # what's pid??? # only the works will be "accepting" requests
        start
      end
    end
    # prevents teh master process from exiting early and leaving zombie processes
    Process.waitall #wait for those processes to exist, otherwise you'll get zombie processes (to prevenet this, use waitall which waits for all all the child processes to exit)
  end
  ...
  def start_em
    EM.run do # eventmahcine provides its own method for running a loop
      # it will take hold of our process and it will get *stuck* here until we stop the event machine
      EM.start_server "localhost", @port, EMConnection do |connection|# EM takes care of initializing and creating the connection / instance of connections here + has caching and pulling and all of that stuff for better performance || EMConnection is a new class
        connection.app = @app
      end
    end
  end
  ...
  class EMConnection < EM::Connection
    attr_accessor :app # we got the app in the #start_em emthod

    def post_init # this will override a callback. modification from #initialize
      @parser = Http::Parser.new(self) 
    end

    def receive_data(data) # this will override a callback. modification from #process
        @parser << data
          # the data is displayed is the following:
          # GET / HTTP/1.1
          # Host: localhost:3000
    ...
    def send_response(env) # make sure that the function takes in the argument
      ...
      headers.each_pair do | name, value| 
        send_data "#{name}: #{value}\r\n" # provided by eventmachine -> replaces the socket write
      end
      send_data "\r\n"  # provided by eventmachine -> replaces the socket write
      
      body.each do | chunk |  
        send_data chunk  # provided by eventmachine -> replaces the socket write
      end

      body.close if body.respond_to? :close

      close_connection_after_writing  # provided by eventmachine -> replaces the socket write
    end
  end
...
server.start_em 3 # instead of start => there are 3 child processes

# then run the ps command inthe command line in order to see all the proesses available
# the computer will then try to balance the load for each child process (the OS is doing some load balancing)
# if you have multiple processors, the server will try to distribute the load for each processor
# you can combine both threads and processes
# we have a prefork threaded sever (but: the more stuff, the more problems / crashes you will have)

# 3 concurrency models:
# 1) threads -> needs to track the context | more memory + context switching 
# 2) prefork model -> 
# 3) eventloop
